    ...
    for genre in genres:
        ...
        for i in range(0,df.shape[0]):
            if genre in df.loc[i,'genres']:
                ...
                m=m+1
        genres_avg[genre]=float(score/m)  
    for category in categories:
        ...
        for i in range(0,df.shape[0]):
            if category in df.loc[i,'categories']:
                score+=df.loc[i,'playtime_forever']
                m=m+1
        categories_avg[category]=float(score/m)
    for tag in tags:
        m=0
        score=0
        for i in range(0,df.shape[0]):
...
if __name__ == "__main__":
    ...
    new_train.to_csv('train_target2.csv',encoding='gbk',index=False)
    new_test.to_csv('test_target2.csv',encoding='gbk',index=False)
